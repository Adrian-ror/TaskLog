import React, {useState} from "react";
import {useTaskStore} from "../../../store/useTaskStore.js";
import {AddButton} from "../../buttons/index.js";
import {ModalBody, ModalHeader, ModalFooter} from "../index.js";
import {priorities, statusOptions} from "../../../config/index.js";

const AddTaskForm = () => {

    const [showModal, setShowModal] = React.useState(false);
    const [title, setTitle] = React.useState('');
    const [priority, setPriority] = useState(priorities.TOP);

    const addTask = useTaskStore(state => state.addTask);

    const handleSubmit = () => {
        setShowModal(false);
        addTask({ title, priority, status: statusOptions.PENDING});
        setTitle('');
    }

    const handlePriorityChange = (name) => {
        if (priorities[name.toUpperCase()]) setPriority(priorities[name.toUpperCase()])
    }


    return (
        <>
            <AddButton work={setShowModal} />

            {showModal && (
                <>
                    <div className="justify-center items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
                        <div className="relative w-full my-6 mx-auto max-w-md">
                            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                                <ModalHeader modalTitle="Add a Task" />
                                <ModalBody
                                    inputValue={title}
                                    handleInput={setTitle}
                                    selectValue={priority.name}
                                    handleSelect={handlePriorityChange}
                                    edit={false}
                                />
                                <ModalFooter
                                    handleClose={setShowModal}
                                    handleSave={handleSubmit}
                                />
                            </div>
                        </div>
                    </div>
                    <div className="opacity-25 fixed inset-0 z-40 bg-black"></div>
                </>
            )}
        </>
    );
}



export default AddTaskForm;